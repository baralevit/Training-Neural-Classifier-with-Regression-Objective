...
class VGG(nn.Module):
    def __init__(self, vgg_name='VGG16'):
        ...
        self.classifier2 = nn.Linear(256,11)
...
def label_to_R5(label):
    ...
    if label == 9:
        r5[9]  =1
    r5[10] = -1
    return r5.unsqueeze(0)


def labels_to_R9(labels):
    matlabels=torch.zeros(batch_size,5)
      
    to_return = label_to_R5(labels[0])
    for j in range(1,batch_size):
        y=labels[j]
        to_return=torch.cat((to_return, label_to_R5(labels[j])))
...
def prediction(outputs):
    ...
    for i in range(batch_size):
        for j in range(10):
            distance[i,j]=torch.dist(outputs[i],label_to_R5[j].to(device),2)
